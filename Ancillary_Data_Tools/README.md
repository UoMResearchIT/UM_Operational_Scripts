...
The conversion to UM file format stage and creates new land sea masks. Note edits to **cci2jules.json** to match the above CCI value and metadata changes. This is adding no_data and ocean classes. Note also ` mkdir cci_maskout`
...
Instead get higher resolution from Powell et al. (2012) [Vegetation and soil carbon‐13 isoscapes for South America: integrating remote sensing and ecosystem isotope measurements - Powell - 2012 - Ecosphere - Wiley Online Library](https://esajournals.onlinelibrary.wiley.com/doi/pdf/10.1890/ES12-00162.1)

```bash
 wget https://github.com/rebeccalpowell/grassmapr-data/raw/master/vegetation-asis/c3_crop_cov_2000_10min_adj.tif .
 wget https://github.com/rebeccalpowell/grassmapr-data/raw/master/vegetation-asis/c4_crop_cov_2000_10min_adj.tif .
```
...
Check fractions add up to 1 by summing over a dimension:

```bash
ncwa -a dim0 -y sum lct_out.nc fraction_check.nc
```

Also check ocean mask worked correctly using ncview.
